...
"/usr/local/lib";
#else
"C:\\Program Files\\Saxonica\\SaxonHEC 11.4";
#endif
...
void setDllname() {
	...
	if (env != NULL) {
		...
		if (snprintf(dllname, bufSize, "%s%s", dllPath, tempDllname) >= bufSize) {
			...
			dllname = malloc(sizeof(char)*bufSize);
			if (snprintf(dllname, bufSize, "%s%s", dllPath, tempDllname) >= bufSize) {
				printf("SaxonC Error: Unable to allocate space for dllname and path");
				exit(1);
			}
		}
		if (snprintf(resources_dir, rbufSize, "%s%s", dllPath, tempResources_dir) >= rbufSize) {
			printf("SaxonC warning: Unable to allocate space for resources directory");
...
void initJavaRT(HANDLE myDllHandle, JavaVM** pjvm, JNIEnv** penv)
{
	if (jvmCreated == 0) {
		jvmCreated = 1;
#ifdef DEBUG
		perror("initJavaRT - load SaxonC library\n");
#endif
		int result;
		JavaVMInitArgs args;
...
#ifdef DEBUG
		perror("initJavaRT - SaxonC library loaded already\n");
#endif
	}
}
...
void setParameter(sxnc_parameter **parameters, int * parLen, int * parCap, const char * namespacei, const char * name, jobject value) {
    ...
    int namespaceLen = 0;
    char *newNamespace =  NULL;
    if(strlen(namespacei)>0) {
        namespaceLen = strlen(namespacei);
        newNamespace = malloc(sizeof(char)*namespaceLen);
