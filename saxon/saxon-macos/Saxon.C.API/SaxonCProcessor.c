...
void executeQueryToFile(sxnc_environment *environi, sxnc_processor ** proc, char * cwd, char* outputfile, sxnc_parameter *parameters, sxnc_property * properties, int parLen, int propLen){
	...
	if(queryFileID == NULL) {
		queryFileID = (jmethodID)(*(environi->env))->GetMethodID (environi->env, cppClass,"executeQueryToFile", "(Ljava/lang/String;Ljava/lang/String;[Ljava/lang/String;[Ljava/lang/Object;)V");
 		if (!queryFileID) {
        		printf("Error: MyClassInDll. executeQueryToFile not found\n");
			fflush (stdout);
			return;
    		} 
	}
...
const char * executeQueryToString(sxnc_environment *environi, sxnc_processor ** proc, char * cwd, sxnc_parameter *parameters, sxnc_property * properties, int parLen, int propLen){
	...
	if(queryStrID == NULL) {
		queryStrID = (jmethodID)(*(environi->env))->GetMethodID (environi->env, cppClass,"executeQueryToString", "(Ljava/lang/String;[Ljava/lang/String;[Ljava/lang/Object;)[B");
		if (!queryStrID) {
    ...
    jobject obj = ((*(environi->env))->CallObjectMethod(environi->env, cpp, queryStrID, (cwd== NULL ? (*(environi->env))->NewStringUTF(environi->env, "") : (*(environi->env))->NewStringUTF(environi->env, cwd)), stringArray, objectArray ));

      (*(environi->env))->DeleteLocalRef(environi->env, objectArray);
      (*(environi->env))->DeleteLocalRef(environi->env, stringArray);
      if(obj) {
          jbyteArray result = (jbyteArray) obj;
          jboolean isCopy = false;
          jbyte* b = (*(environi->env))->GetByteArrayElements(environi->env, result, &isCopy);
          jsize num_bytes = (*(environi->env))->GetArrayLength(environi->env, result);

          char * str = malloc(sizeof(char)*(num_bytes + 1));
          memcpy ( str, b , num_bytes );
          str[num_bytes] = '\0';

          (*(environi->env))->ReleaseByteArrayElements(environi->env, result, b, 0);
          (*(environi->env))->DeleteLocalRef(environi->env, obj);
          return str;
