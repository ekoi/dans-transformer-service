...
public:
   ...
   void setcwd(const char* cwd);
      ...
      //! Set the base output URI.
        ...
        <p> The base output URI is used for resolving relative URIs in the <code>href</code> attribute
           ...
           * code using the XPath current-output-uri() function</p>
           *
    ...
    */
    ...
    /**
     *
     * @param modeName the EQName of the initial mode. Two special values are recognized, in the
    ...
    */
    ...
    /**
    *   Return the result-documents resulting from the execution of the stylesheet.
    *   Null is returned if the user has not enabled this feature via the method setCaptureResultDocuments()
    *   @return std::map - the map is indexed by the absolute URI of the result document, and the corresponding
    *   value is an XdmNode containing the result document (as an in-memory tree, without serialization)
    */
   ...
   /**
     * If this option is enabled, then any document created using xsl:result-document is saved (as an XdmNode)
     * in an std::map object where it is accessible using the URI as a key. After the execution of the transformation
     * a call on the <code>getResultDocuments()</code> method is required to get access to the result-documents in the map.
     * @param flag - true causes secondary result documents from the transformation to be saved in a map; false disables
     * this option.
     * @param rawResults - true enables the handling of raw destination for resultsDocuments. If not supplied this can also be set on the setResultAsRawValue method.
     * The setResultAsRawValue method has higher priority to this flag
    */
    ...
    /**
     * The representation of the compiled stylesheet is in an XML form, suitable for
     ...
     *
     * @param filename - the destination for the XML document containing the diagnostic representation
    ...
    */
     ...
     * @param source - source document supplied as an XdmNode object. nullptr accepted if source document has been set already.
    ...
    /**
     * Perform the transformation based upon a compiled stylesheet and source document.
     * Assume the output file has been set in advance
     * @param source - source document supplied as an XdmNode object. nullptr accepted if source document has been set already.
     *
...
private:
 //! Default constructor.
    ...
    */
    XsltExecutable(jobject exObject, std::string cwd, std::string resources);
	...
	jobject executableObject, selection, saxonMessageListenerObj, saxonResultDocObj;
	XdmValue * selectionV;
    std::string cwdXE; /*!< current working directory */
    std::string resources_dir; /*!< current Saxon resources directory */
	bool tunnel, jitCompilation, rawResultsFlag;
