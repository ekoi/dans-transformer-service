...
SchemaValidator::SchemaValidator(SaxonProcessor* p, std::string curr){
	...
	if ((proc->proc) == nullptr) {
	    ...
	    jobject tempcppV = createSaxonProcessor2(SaxonProcessor::sxn_environ->env, cppClass,
    			"(Lnet/sf/saxon/s9api/Processor;)V", proc->proc);


        if(tempcppV) {
            cppV = SaxonProcessor::sxn_environ->env->NewGlobalRef(tempcppV);
            SaxonProcessor::sxn_environ->env->DeleteLocalRef(tempcppV);

        } else {
            std::cerr<<"Error: Failed to create the internal object for the SchemaValidator"<<std::endl;
  ...
  void SchemaValidator::registerSchemaFromString(const char * sourceStr){
	...
	if (size > 0) {
		...
		for (std::map<std::string, std::string>::iterator iter = properties.begin();
			...
			SaxonProcessor::sxn_environ->env->CallVoidMethod(cppV, mID,
					SaxonProcessor::sxn_environ->env->NewStringUTF(cwdV.c_str()),
					(sourceStr != nullptr ? SaxonProcessor::sxn_environ->env->NewStringUTF(sourceStr) : NULL), NULL, stringArray, objectArray);
...
const char * SchemaValidator::getErrorMessage(int i ){
 if(proc->exception == nullptr) {return nullptr;}
 return proc->exception->getMessage();
 }
