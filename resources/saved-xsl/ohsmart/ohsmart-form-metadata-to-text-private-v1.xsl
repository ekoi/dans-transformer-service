    ...
    <xsl:template match="/" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        ...
        <xsl:for-each select="//array[@key='metadata']/map">
            ...
            <xsl:for-each select="array[@key='fields']/map">
                <xsl:choose>
                    <xsl:when test="string[@key='name' and (text()='interviewee' or text()='interviewer' or text()='interpreter' or text()='others' or text()='transcript_human')]">
                        <xsl:choose>
                            ...
                            <xsl:when test="string[@key='type' and text()='group'] and boolean[@key='repeatable' and text()='true']">
                                ...
                                <xsl:for-each select="array[@key='fields']/array">
                                    <xsl:for-each select="map">
                                        <xsl:variable name="GroupItemName" select="string[@key='name']"/>
                                        <xsl:choose>
                                            ...
                                            <xsl:when test="array[@key='fields'] and string[@key='type' and text()='repeatSingleField']">
                                                <xsl:variable name="SubReapeatFieldName" select="string[@key='name']"/>
                                                <xsl:for-each select="array[@key='fields']/map">
                                                    <xsl:text>&#10;</xsl:text>
                                                    <xsl:value-of select="$Section" />
                                                    <xsl:text>.</xsl:text>
                                            ...
                                            <xsl:when test="array[@key='fields']">
                                                <xsl:variable name="GroupName2" select="string[@key='name']"/>
                                                <xsl:for-each select="array[@key='fields']/map">
                                                    <xsl:text>&#10;</xsl:text>
                                                    <xsl:value-of select="$Section" />
                                                    <xsl:text>.</xsl:text>
                            ...
                            <xsl:when test="array[@key='fields'] and string[text()='repeatSingleField']">
                                <xsl:variable name="RepeatableSingleField" select="string[@key='name']"/>
                                <xsl:for-each select="array[@key='fields']/map">
                                    <xsl:text>&#10;</xsl:text>
                                    <xsl:value-of select="$Section" />
                                    <xsl:text>.</xsl:text>
                            ...
                            <xsl:when test="array[@key='fields']">
                                ...
                                <xsl:for-each select="array[@key='fields']/map">
                                    <xsl:variable name="GroupSubName3" select="string[@key='name']"/>
                                    <xsl:choose>
                                        <xsl:when test="string[@key='value']">
                                            <xsl:text>&#10;</xsl:text>
                                            <xsl:value-of select="$Section" />
        ...
        <xsl:for-each select="//array[@key='file-metadata']/map">
            <xsl:variable name="isExcluded" select="boolean(boolean[@key='private' and text()='false'])"/>
            <xsl:choose>
