...
<xsl:stylesheet xmlns:dc="http://purl.org/dc/elements/1.1/"
    ...
    <xsl:template match="map" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        ...
        <xsl:for-each select="//array[@key = 'metadata']/map">
            <xsl:choose>
                <xsl:when
                    test="string[@key = 'name' and (text() = 'interviewee' or text() = 'interviewer' or text() = 'interpreter' or text() = 'others' or text() = 'transcript_human')]">
                    <xsl:text>"</xsl:text>
                ...
                <xsl:choose>
                    <xsl:when
                        ...
                        <xsl:choose>
                            ...
                            <xsl:when test="array[@key = 'fields']">
                                <xsl:text>{ </xsl:text>
                                <xsl:for-each select="array[@key = 'fields']/map[boolean[@key = 'private' and . = 'true']]">
                                    <xsl:text>"</xsl:text>
                                    <xsl:value-of select="string[@key = 'name']"/>
                                    <xsl:text>": </xsl:text>
                                    <xsl:choose>
                        ...
                        <xsl:choose>
                            <xsl:when
                                test="position() != last() and not(string[@key = 'name' and text() = 'transcript_human'])">
                                <xsl:text>, </xsl:text>
                            </xsl:when>
                        </xsl:choose>
                    </xsl:when>
        ...
        <xsl:for-each select="//array[@key = 'file-metadata']/map">
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    ...
                    <xsl:text>}</xsl:text>
                    <xsl:if
                        test="following-sibling::map[boolean[@key = 'private' and text() = 'true']]">
                        <xsl:text>, </xsl:text>
                    </xsl:if>
                </xsl:otherwise>
            </xsl:choose>
